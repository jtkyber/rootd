...
const PsgSelector = ({ textArea, setAddingPsg, psgSelectorRef }) => {
	...
	const handleInputChange = e => {
		...
		if (starting) {
			if (e.target.classList.contains(styles.chapter)) {
				...
				if (
					input === 0 ||
					input <= parseInt(books[bookIndex].chapters[psgContent.starting.chapter - 1]?.verses)
				) {
			...
			if (e.target.classList.contains(styles.chapter)) {
				...
				if (
					input === 0 ||
					input <= parseInt(books[bookIndex].chapters[psgContent.ending.chapter - 1]?.verses)
				) {
	...
	const handleBookInputChange = e => {
		...
		books.forEach((book, i) => {
			if (e.target.value.length && book.book.toLowerCase().includes(e.target.value.toLowerCase()))
				return filteredBooks.push(i)
			return
		})
	...
	const handleSubmit = e => {
		...
		if (
			...
			if (
				(psgContent.ending.chapter > 0 &&
					psgContent.ending.verse > 0 &&
					psgContent.ending.chapter < psgContent.starting.chapter) ||
				(psgContent.ending.chapter === psgContent.starting.chapter &&
					psgContent.ending.verse <= psgContent.starting.verse)
			)
				return
			const passage =
				psgContent.ending.chapter > 0 && psgContent.ending.verse > 0
			...
			const passageHTML = `<span class='passageLink' contentEditable='false' id=${passage.replaceAll(
				' ',
				'%20'
			)}>${passage}</span>`
	...
	return (
		<form
			...
			<div className={styles.optionContainer}>
				<div
					ref={bookResultsRef}
					className={styles.bookResults}>
					{bookResults.map((bookIndex, i) => {
						return (
							<h5
								onClick={() => setSelectedBook(bookIndex)}
								key={i}
				...
				{starting ? (
					<input
						...
						className={`${styles.option} ${styles.book}`}
						type='text'></input>
				) : null}
				<input
					disabled={!psgContent.book.length}
					placeholder='Chapter'
					onChange={e => handleInputChange(e)}
					value={
						starting
							? psgContent.starting.chapter > 0
								? psgContent.starting.chapter
								: ''
							: psgContent.ending.chapter > 0
							? psgContent.ending.chapter
							: ''
					}
					className={`${styles.option} ${styles.chapter}`}
					type='text'></input>
				<input
					disabled={!psgContent.book.length}
					placeholder='Verse'
					...
					value={
						starting
							? psgContent.starting.verse > 0
								? psgContent.starting.verse
								: ''
							: psgContent.ending.verse > 0
							? psgContent.ending.verse
							: ''
					}
					className={`${styles.option} ${styles.verse}`}
					type='text'></input>
			</div>
