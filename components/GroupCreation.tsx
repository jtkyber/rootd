			...
			case 0:
				values?.name?.trim()?.length
					? values.name.length < 50
						? goToNextSection(1)
						: setErrMsg('Name must be under 50 characters')
		...
		<div className={styles.container}>
			<div className={styles.groupCreationContainer}>
				{!groupRequestSent ? (
								...
								{sectionIndex === 0 ? (
											...
											onSelection={e =>
												setValues({
													...
													books: e.target.checked
														? [...values.books, e.target.value]
														: values.books.filter(item => item !== e.target.value),
												})
											}
									...
									<>
										<h5 className={styles.instructions}>
											Select the characters(s) involved in this group (optional)
										</h5>
										<DropDown
											key='characters2'
											idEnd='characters2'
											...
											onSelection={e =>
												setValues({
													...
													characters: e.target.checked
														? [...values.characters, e.target.value]
														: values.characters.filter(item => item !== e.target.value),
												})
											}
									...
									<>
										<h5 className={styles.instructions}>
											Add 2-5 tags related to your group (this will make your group easier to find)
										</h5>
														...
														onClick={e =>
															setValues({
																...
																tags: values.tags.filter(
																	item => item !== (e.target as HTMLHeadingElement).innerText
																),
															})
														}
					...
					<h3 className={styles.creationReqConfirmationMsg}>
						Group creation request has been sent. You will be notified as soon as the group is accepted
					</h3>
				)}
			</div>
		</div>
