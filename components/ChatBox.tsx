...
const ChatBox = ({ channels }: { channels: PresenceChannel[] | [] }) => {
	...
	const sendMessage = useMutation(
		(msgData: Partial<IMessage> & ObjectId) => {
		...
		},
		{
			onSuccess: async newData => {
				...
				await axios.get(
					`/api/pusher/updateGrpMemberMsgs?username=${user.username}&channelName=presence-${
						selectedGroup._id
					}&msg=${JSON.stringify(newData)}`
				)
			},
		}
	)
	...
	const refetch = () =>
		queryClient.resetQueries({ queryKey: ['groupMessages', selectedGroup._id], type: 'active' })
	...
	useEffect(() => {
		...
		channels?.[1].bind('update-online-members', data => {
			const onlineMembs = Object.keys(channels?.[1].members.members).map(
				key => channels?.[1].members.members[key].username
			)
	...
	const sendJoinedGroup = async () => {
		await axios.get(
			`/api/pusher/updateMemberStatus?username=${user.username}&channelName=${selectedGroup._id}`
		)
	}
	...
	const handlePageClick = e => {
		if (e.target.classList.contains('passageLink')) {
			window.open(
				`https://www.biblegateway.com/passage/?search=${e.target.id}&version=${user.bVersion}`,
				'_blank'
			)
		}
	}
	...
	async function fetchGroupMessages({ pageParam = 0 }) {
		...
		try {
			const res = await axios.get(
				`/api/getGroupMsgs?groupId=${selectedGroup._id}&cursor=${pageParam}&limit=${resultsLimit}`
			)
			return res.data
		} catch (err) {
			console.log(err)
							...
							return {
								...
								likes:
									m._id === msgId
										? isAdded
											? [...m.likes, liker]
											: m.likes.filter(like => like !== liker)
										: m.likes,
							}
						}),
					}
	...
	return (
		<div className={`${styles.selectedGroup}`}>
			...
			<div
							...
							<Fragment key={i}>
								{page?.data.map((msg: IMessage, j, row2) => {
                                    ...
                                    `}>
											<div className={stylesChat.msgAuthor}>
												{msg?.authorProfileImg ? (
													<Image
														onClick={() =>
															msg.authorId !== user._id
																? setSelectedMember({ id: msg.authorId, img: msg.authorProfileImg })
																: null
														}
														width={25}
														height={25}
														className={`${stylesChat.authorImg} ${
															onlineMembers.includes(msg.author) ? stylesChat.online : null
														}`}
													...
													<h5
														...
														className={`${stylesChat.authorImg} ${stylesChat.noImg} ${
															onlineMembers.includes(msg.author) ? stylesChat.online : null
														}`}>
											...
											<h4
												onClick={e => showLikeNames(e, msg)}
												className={stylesChat.msgContent}>
											...
											<div
												...
												{msg.likes.map((name, i) => {
													return (
														<h6
															key={i}
															className={stylesChat.likeName}>
															{name}
															<Image
																className={stylesChat.likeImg}
																src={LikeIconSVG}
																alt='Like Icon'
															/>
														</h6>
					...
					{addingPsg ? (
						<>
							<PsgSelector
								...
								setAddingPsg={setAddingPsg}
								psgSelectorRef={psgSelectorRef}
							/>
							<svg
								className={stylesChat.triangle}
								ref={svgRef}
								width={windowWidth}
								height={windowHeight}>
								<polygon
									points={`${pointOneX},${pointOneY} ${pointTwoX},${pointTwoY} ${pointThreeX},${pointThreeY}`}
									fill='rgb(255,0,0)'
									stroke='rgb(255,0,0)'
									strokeWidth='2'
								/>
				...
				<div
					style={{ pointerEvents: addingPsg ? 'none' : 'all' }}
					className={stylesChat.chatInputArea}>
					<button
						onClick={() => setAddingPsg(true)}
						className={`${stylesChat.addPsgBtn} hasTooltip`}
						data-tooltip-msg='Insert Passage'
					...
					<div
						...
						className={stylesChat.input}></div>
					<button
