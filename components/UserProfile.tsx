...
const UserProfile = ({
	...
	selectedMember: ISelectedMember
	...
	const handlePageClick = e => {
		if (
			...
			!inviteFormRef?.current?.contains(e.target) &&
	...
	const userHasGroupsThatMemberDoesNot = (): boolean => {
		const groupCopy = userGroups.filter(g => {
			...
			return g?._id &&
				!sharedGroupIdArray?.includes(g._id.toString()) &&
				...
				<>
					{!inviting ? (
						<div
							...
							<div className={styles.msgAuthor}>
								{selectedMember?.img ? (
									...
									<h5 className={`${styles.authorImg} ${styles.noImg}`}>
										{getInitials(memberDetails.username)}
									</h5>
								)}
								<h2>{memberDetails.username}</h2>
							</div>
							<h4 className={styles.preferedBibleVersion}>
								Prefered Bible Version: {memberDetails.bVersion}
							</h4>
							...
							<div className={styles.bottomSection}>
								<button
									onClick={sendDm}
									className={styles.sendDmBtn}>
									Send direct message
								</button>
								<button
									onClick={() => (userHasGroupsThatMemberDoesNot() ? setInviting(true) : null)}
									className={styles.inviteToGroup}>
									Invite to group
								</button>
							</div>
						</div>
						...
						<form
							onSubmit={sendInvite}
							ref={inviteFormRef}
							className={styles.invitingToGroup}>
										...
										return (
											<div
												...
												<input
													...
													value={group.name}
												/>
												<label htmlFor={group._id.toString()}>{group.name}</label>
												<br></br>
											</div>
										)
									})}
							...
							<button
								type='submit'
								className={styles.submitInviteBtn}>
								Send Invite
							</button>
						</form>
					)}
