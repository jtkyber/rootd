...
const NotificationCenter = () => {
	...
	useEffect(() => {
		const notifArrTemp: JSX.Element[] = user?.notifications
			?.slice()
			?.sort((a, b) => Date.parse(b.date) - Date.parse(a.date))
			?.map((notif: INotification, i: number) => {
				return (
					<div
						onClick={e => onNotificationClick(e, notif)}
						key={i}
						className={`${styles.notification} ${notif.read ? styles.read : null}`}>
						{notif?.notificationType === 'message-like' && notif?.likers?.length && notif?.group ? (
							<h5>
								{notif.likers[0]}{' '}
								{notif.likers.length > 1
									? 'and ' + (notif.likers.length - 1) + ` other${notif.likers.length > 2 ? 's' : ''}`
									: ''}{' '}
								liked your message in "{notif.group.name}"
							</h5>
							...
							<div className={styles.groupInviteNotif}>
								<h5 className={styles.text}>
									{notif.inviter} invited you to join "{notif.group?.name}"
								</h5>
								{!notif.read ? (
									<>
										<button
	...
	const onNotificationClick = async (e, notification: INotification) => {
		if (
			notification.notificationType === 'group-invite' &&
			e.target?.id !== 'reject' &&
			e.target?.id !== 'join'
		)
			return
		...
		switch (notification.notificationType) {
			...
			case 'dm-like':
				await markNotificationAsRead(notification, true)
				if (notification?.likers?.[0] && notification?.likerId)
					dispatch(
						setSelectedDmPerson({ _id: notification.likerId.toString(), username: notification.likers[0] })
					)
		...
		<div className={styles.notificationContainer}>
			...
			<div
				className={`${styles.notificationDropdown} ${
					activeDropdown === 'notifications' ? styles.active : null
				}`}>
				{notificationArray}
			</div>
		</div>
