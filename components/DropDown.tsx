...
const DropDown = ({ idEnd, name, optionArray, onSelection, selectedValues }: IParams) => {
    useEffect(() => { if (selectedValues) applySelectedValues() }, [])
    ...
    return (
        <div className={`${styles.selector}`}>
            <button onClick={toggleList} className={styles.selectorBtn}>{name}</button>
            <div ref={selectorRef} className={styles.options}>
            {
                optionArray.map((option, i) => (
                <div key={i} className={styles.checkboxChunk}>
                    <input type='checkbox' onChange={onSelection} id={option + `-${idEnd}`} name={option} value={option} />
