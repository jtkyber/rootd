...
const admin = () => {
	...
	const refetch = () =>
		queryClient.resetQueries({ queryKey: ['adminNotifications', [resultType]], type: 'active' })
	...
	useEffect(() => {
		if (user?._id) setResultType('groupCreationRequests')
	}, [user._id])

	useEffect(() => {
		if (
			data?.pages[data?.pages.length - 1]?.cursor === resultsLimit ||
			data?.pages[data?.pages.length - 1]?.cursor === null
		) {
	...
	async function fetchResults({ pageParam = 0 }) {
		...
		try {
			const res = await axios.get(
				`/api/admin/fetchAdminNotificationResults?userId=${user._id}&resultType=${resultType}&cursor=${pageParam}&limit=${resultsLimit}`
			)
			return res.data
		} catch (err) {
			console.log(err)
			...
			<>
				<div className={styles.notification}>
					<h4 className={styles.notificationTitle}>
						{resultType == 'groupCreationRequests' ? 'Group Creation Request' : null}
					</h4>
					...
					<div className={styles.notificationMsg}>
						{resultType == 'groupCreationRequests' ? (
							<>
								<h5>
									<span>Name:</span> {selectedResult?.name}
								</h5>
								<h5>
									<span>Description:</span> {selectedResult?.description}
								</h5>
								<h5>
									<span>Tags:</span> {selectedResult?.tags?.join(', ')}
								</h5>
								<h5>
									<span>Date of request:</span> {getFormattedDateFromISO(selectedResult?.date)}
								</h5>
							</>
						) : null}
					</div>
				...
				<>
					...
					<div className={styles.sideBar}>
						<div className={styles.options}>
							<h5
								className={`${styles.resultTypeBtn} ${
									resultType === 'groupCreationRequests' ? styles.active : ''
								}`}
								id='groupCreationRequests'
								onClick={e => setResultType((e.target as HTMLButtonElement).id)}>
								New Groups
							</h5>
							<h5
								className={`${styles.resultTypeBtn} ${resultType === 'userReports' ? styles.active : ''}`}
								id='userReports'
								onClick={e => console.log('Not yet implemented')}>
								User Reports
							</h5>
							<h5
								className={`${styles.resultTypeBtn} ${resultType === 'groupReports' ? styles.active : ''}`}
								id='groupReports'
								onClick={e => console.log('Not yet implemented')}>
								Group Reports
							</h5>
								...
								{data?.pages?.map((page, i) => (
									<Fragment key={i}>
										{page?.data?.map((notif, j) => {
											return (
												<div
													onClick={() => setSelectedResult(notif)}
													className={`${styles.result} ${
														notif._id === selectedResult._id ? styles.active : null
													}`}
													key={j}>
					...
					<div className={styles.content}>{selectedResult._id ? constructResultContent() : null}</div>
				</>
			) : (
				<NotAuthorizedScreen />
			)}
