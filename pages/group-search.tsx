...
const groupSearch = () => {
	...
	const { data, status, isFetching, fetchNextPage, hasNextPage, error } = useInfiniteQuery(
		['groups', [options, currentSort, page]],
		fetchGroups,
		{
	...
	async function fetchGroups({ pageParam = 0 }) {
		try {
			const res = await axios.get(
				`/api/findGroups?keyword=${options.keyword}&characters=${JSON.stringify(
					options.characters
				)}&books=${JSON.stringify(options.books)}&includePrivate=${options.includePrivate}&sortType=${
					currentSort.name
				}&sortDir=${currentSort.dir}&cursor=${pageParam}&limit=${resultsLimit}`
			)
			return res.data
		} catch (err) {
			console.log(err)
	...
	const onCharacterChange = e => {
		e.target.checked
			...
			: setOptions({
					...options,
					characters: [...options.characters.filter(item => item !== e.target.value)],
			  })
	}
	...
	function getTimeDiff(lastActive) {
		...
		if (timeSinceLastActive < 60) {
			return `${Math.round(timeSinceLastActive)}min${includeS(timeSinceLastActive)}`
		} else timeSinceLastActive /= 60
		// is hours
		if (timeSinceLastActive >= 1 && timeSinceLastActive < 24) {
			return `${Math.round(timeSinceLastActive)}hr${includeS(timeSinceLastActive)}`
		} else timeSinceLastActive /= 24
		// is days
		if (timeSinceLastActive >= 1 && timeSinceLastActive < 365) {
			return `${Math.round(timeSinceLastActive)}dy${includeS(timeSinceLastActive)}`
		} else timeSinceLastActive /= 365
		// is years
		if (timeSinceLastActive >= 365)
			return `${Math.round(timeSinceLastActive)}yr${includeS(timeSinceLastActive)}`
				...
				<>
					<div className={styles.searchParams}>
						<input
							...
							onChange={e => onKeywordChange(e.target.value.toLowerCase())}
							type='text'
					...
					<div className={styles.sortContainer}>
						<h6
							...
							className={styles.nameSort}>
							Name {currentSort.name === 'name' && currentSort.dir === 'up' ? '▲' : '▼'}
						</h6>
						...
						<div className={styles.sortRightChunk}>
							<h6
								...
								Members {currentSort.name === 'members' && currentSort.dir === 'up' ? '▲' : '▼'}
							</h6>
							<h6
								id='lastActive'
								onClick={e => handleSortClick(e)}
								className={styles.lastActiveSort}>
								Last Active {currentSort.name === 'lastActive' && currentSort.dir === 'up' ? '▲' : '▼'}
							</h6>
							<h6
								id='isPrivate'
								onClick={e => handleSortClick(e)}
								className={styles.privateSort}>
								Private {currentSort.name === 'isPrivate' && currentSort.dir === 'up' ? '▲' : '▼'}
							</h6>
						</div>
					</div>
									...
									return (
										<div
											...
											{group?._id && user?.groups.includes(group._id.toString()) ? (
												...
												<button
													onClick={() =>
														group.isPrivate && user?.username !== group.groupAdmin
					...
					{activePrivateGroup ? (
						<Confirmation
							exitFunction={() => setActivePrivateGroup(null)}
							enterFunction={password => handleJoinGroup(activePrivateGroup, password)}
						/>
					) : null}
				</>
			) : (
