...
export default async function handler(req: NextApiRequest, res: NextApiResponse<Partial<IUser | any>>) {
	try {
		const { username, password, email, gender, bVersion } = req.body
		await connectMongo()
		if (username == null || password == null || email == null || gender == null || bVersion == null)
			throw new Error('Please fill out all fields')
		else if (gender === 'Gender' || bVersion === 'Prefered Bible Version')
			throw new Error('Please fill out all fields')
		...
		else if (email.split('@').length - 1 !== 2 && !email.charAt(email.indexOf('@') + 1))
