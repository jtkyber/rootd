...
export default async function handler(req: NextApiRequest, res: NextApiResponse<any>) {
	try {
		...
		const { notificationType, groupId, groupName, msgId, newLiker, userId, userName, inviter, reason }: any =
			req.body
				...
				User.updateOne(
					...
					if (docs.modifiedCount > 0) {
						...
						if (pusherRes.status === 200) {
							...
							if (body?.occupied) {
								await pusher.trigger(userId, 'update-notifications', {
									notificationType: notificationType,
									notification: JSON.stringify(await getNotifications()),
								})
							}
						}
						res.json(true)
